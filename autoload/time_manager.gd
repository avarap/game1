...
func advance_to_next_day(wake_hour: int = DEFAULT_WAKE_HOUR, wake_minute: int = DEFAULT_WAKE_MINUTE) -> void:
    day += 1
    var normalized := TimeMath.normalize_total_minutes(TimeMath.to_total_minutes(wake_hour, wake_minute))
    hour = normalized.hour
    minute = normalized.minute
    EventBus.day_changed.emit(day)
    EventBus.time_changed.emit(hour, minute)
