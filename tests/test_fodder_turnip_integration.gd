...
static func run() -> Array[String]:
	...
	if turnip == null or seed == null:
		failures.append("Fodder turnip items should load")
		return failures
	if turnip.id != &"fodder_turnip" or seed.id != &"fodder_turnip_seed":
		failures.append("Fodder turnip item IDs should be stable")
	if merchant == null or not merchant.is_valid():
		failures.append("Yard supplier should remain valid")
		return failures
	...
	if seed_offer == null or turnip_offer == null:
		failures.append("Yard supplier should trade fodder turnips")
		return failures
	if turnip_offer.sell_price_copper <= 0 or seed_offer.buy_price_copper <= 0:
		failures.append("Fodder turnip prices should be positive")
	if seed_offer.buy_price_copper >= turnip_offer.sell_price_copper * 2:
		failures.append("Growing should beat buying equivalent produce")
	...
	if not buy_tx.is_success() or not buy_applied:
		failures.append("Seed purchase should use atomic economy")
	...
	if not seller_tx.is_success() or not sell_applied:
		failures.append("Turnip sale should use atomic economy")

	if recipe == null or not recipe.is_valid():
		failures.append("Fodder turnip recipe should validate")
		return failures
	...
	if result != CraftingService.RESULT_OK:
		failures.append("Turnip recipe should reuse crafting")
	if inventory.count_item(&"fodder_turnip") != 0:
		failures.append("Turnip recipe should consume produce once")
	return failures
