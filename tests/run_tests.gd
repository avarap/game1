...
func _run_all_tests() -> void:
	...
	_run_suite("CemeteryStructuralLandmarks", TestCemeteryStructuralLandmarks.run(), failures)
	_run_suite(
		"CemeteryInteractionReadability",
		await TestCemeteryInteractionReadability.run(),
		failures,
	)
	...
	_run_suite("BuildingArtProduction", TestBuildingArtProduction.run(), failures)
	...
	_run_suite("Phase8AAcceptance", TestPhase8AAcceptance.run(), failures)
