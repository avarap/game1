...
static func run() -> Array[String]:
	...
	if service.finalize_corpse(&"cremate_once", &"cremate") != CemeteryService.RESULT_ALREADY_FINALIZED:
	...
	if technology.get_points(TechnologyService.PointType.BLUE) != decisions.research_blue_points:
		failures.append("Research should grant its configured differentiated reward")
	if service.finalize_corpse(&"research_once", &"cremate") != CemeteryService.RESULT_ALREADY_FINALIZED:
	...
	var restored := CemeteryService.from_snapshot(config, snapshot, decisions, restored_technology)
	...
	if restored.finalize_corpse(&"cremate_once", &"cremate") != CemeteryService.RESULT_ALREADY_FINALIZED:
